...
--shuimeng
local shuimeng_skill = {}
shuimeng_skill.name = "shuimeng"
table.insert(sgs.ai_skills, shuimeng_skill)
shuimeng_skill.getTurnUseCard = function(self)
	if self:willShowForDefense() and not self:isWeak() and not self.player:hasUsed("ShuimengCard") and not self.player:isKongcheng() then return sgs.Card_Parse("@ShuimengCard=.&shuimeng") end
end

sgs.ai_skill_use_func.ShuimengCard = function(ShuimengCard, use, self)
	if #self.friends == 0 then return end
	if self.player:getHandcardNum() < 2 then return end
	local handcards = self.player:getHandcards()
	local cardstable = {}
	for _, c in sgs.qlist(self.player:getHandcards()) do
		if self:getKeepValue(c) <= 3.2 then
			table.insert(cardstable, c)
		end
	end
...
table.insert(sgs.ai_choicemade_filter.cardUsed, shuimeng_filter)

sgs.ai_skill_playerchosen.shuimeng = sgs.ai_skill_playerchosen.jieming
sgs.ai_playerchosen_intention.shuimeng = 80

sgs.ai_card_intention.ShuimengCard = 0
sgs.dynamic_value.benefit.ShuimengCard = true

sgs.ai_skill_discard.shuimeng = function(self, discard_num, min_num, optional, include_equip)
	if not self:willShowForDefence() then return {} end
	return self:askForDiscard("dummy_reason", discard_num, min_num, optional, include_equip)
end

local mingming_skill = {}
mingming_skill.name = "mingming"
table.insert(sgs.ai_skills, mingming_skill)

mingming_skill.getTurnUseCard = function(self)
	if self.player:hasUsed("MingmingCard") then return nil end
	...
	if not self.player:hasShownSkill("mingming") and not willShow then return nil end
		...
		for _, fcard in ipairs(cards) do
			...
			if not fvalueCard then
				...
				for _, scard in ipairs(cards) do
					...
					if first_card ~= scard and scard:isBlack() then
