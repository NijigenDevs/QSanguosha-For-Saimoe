...
local t = {
	...
	["double-sword-card"] = "%src 发动了【旭日之心】效果，你须弃置一张手牌，或令 %src 摸一张牌",
	...
	["SilverLion"] = "塞尔提头盔",
	[":SilverLion"] = "装备牌·防具<br />技能：<font color=\"blue\"><b>锁定技，</b></font>每当你受到多于1点的伤害时，你防止多余的伤害；<font color=\"blue\"><b>锁定技，</b></font>每当你失去装备区里的【塞尔提头盔】时，你回复1点体力。",
	...
	["amazing_grace"] = "收获祭",
	[":amazing_grace"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：所有角色。<br />执行动作：（使用时）你从亮出牌堆顶的X张牌（X为全场角色的数量）。<br />作用效果：每名目标角色获得这些牌中（剩余）的任意一张。",

	["god_salvation"] = "圣灵术",
	[":god_salvation"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：所有角色。<br />作用效果：每名目标角色回复1点体力。",

	["savage_assault"] = "诸神黄昏",
	[":savage_assault"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：所有其他角色。<br />作用效果：每名目标角色需打出一张【杀】，否则受到1点伤害。",
	["savage-assault-slash"] = "%src 使用了【诸神黄昏】，请打出一张【杀】来响应",

	["archery_attack"] = "万箭齐发",
	[":archery_attack"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：所有其他角色。<br />作用效果：每名目标角色需打出一张【闪】，否则受到1点伤害。",
	["archery-attack-jink"] = "%src 使用了【万箭齐发】，请打出一张【闪】以响应",

	["collateral"] = "借刀杀人",
	[":collateral"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：装备区里有武器牌且其攻击范围内有使用【杀】的合法目标的一名其他角色。<br />执行动作：你（在选择该角色为目标的同时）选择目标角色攻击范围内的使用【杀】的一个合法目标A。<br />作用效果：目标角色需对A使用一张【杀】，否则将装备区里的武器牌交给你。",
	["collateral-slash"] = "%dest 使用了【借刀杀人】，请对 %src 使用一张【杀】",
	["#CollateralSlash"] = "%from 选择了此【<font color=\"yellow\"><b>杀</b></font>】的目标 %to",

	["duel"] = "决斗",
	[":duel"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：一名其他角色。<br />作用效果：由目标角色开始，其与你轮流打出一张【杀】，直到其中一方未打出【杀】为止。未打出【杀】的一方受到另一方造成的1点伤害。",
	["duel-slash"] = "%src 对你【决斗】，你需要打出一张【杀】",

	["ex_nihilo"] = "无中生有",
	[":ex_nihilo"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：包括你在内的一名角色。<br />作用效果：目标角色摸两张牌。",

	["snatch"] = "顺手牵羊",
	[":snatch"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：距离为1且区域里有牌的一名角色。<br />作用效果：你获得目标角色区域里的一张牌。",

	["dismantlement"] = "过河拆桥",
	[":dismantlement"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：区域里有牌的一名其他角色。<br />作用效果：你弃置目标角色区域里的一张牌。",

	["nullification"] = "无懈可击",
	[":nullification"] = "法术牌<br />使用时机：一张法术牌对一个目标生效前。<br />使用目标：此牌。<br />作用效果：抵消此牌对该目标产生的效果。",

	["heg_nullification"] = "无懈可击·国",
	[":heg_nullification"] = "法术牌<br />使用时机：1、一张法术牌对一个目标生效前；2、一张法术牌对一名目标角色生效前。<br />使用目标：此牌。<br />作用效果：1、抵消此牌对该目标产生的效果；2、抵消此牌对该目标角色以及与其势力相同的所有尚未结算的目标角色产生的效果。",
	["heg_nullification:single"] = "为该角色无懈",
	["heg_nullification:all"] = "为该势力无懈" ,
	
	["indulgence"] = "禁锢枷锁",
	[":indulgence"] = "延时法术牌<br />使用时机：出牌阶段。<br />使用目标：一名其他角色。<br />作用效果：目标角色进行判定，若结果不为<font color=\"red\">♥</font>，目标角色跳过出牌阶段。",

	["lightning"] = "闪电",
	[":lightning"] = "延时法术牌<br />使用时机：出牌阶段。<br />使用目标：你。<br />目标角色进行判定，若结果为黑桃2~9，则目标角色受到3点无来源的雷电伤害，然后将此【闪电】置入弃牌堆。",

	["iron_chain"] = "心灵链环",
	[":iron_chain"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：一至两名角色。<br />每名目标角色选择一项：1.横置武将牌；2. 重置武将牌。<br />◆你可以重铸此牌。",

	["fire_attack"] = "异端审判",
	[":fire_attack"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：有手牌的一名角色。<br />作用效果：目标角色展示一张手牌，然后你可以弃置与此牌花色相同的一张手牌，若如此做，该角色受到1点火焰伤害。",
	["fire-attack-card"] = "您可以弃置一张与 %dest 所展示卡牌相同花色(%arg)的牌对 %dest 造成1点火焰伤害",
	["@fire-attack"] = "%src 展示的牌的花色为 %arg，请弃置一张与其相同花色的手牌",

	["supply_shortage"] = "提不起劲",
	[":supply_shortage"] = "延时法术牌<br />使用时机：出牌阶段。<br />使用目标：距离为1的一名角色。<br />作用效果：目标角色进行判定，若结果不为♣，目标角色跳过摸牌阶段。",
	
	["await_exhausted"] = "以逸待劳",
	[":await_exhausted"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：你和与你势力相同的所有角色。<br />作用效果：每名目标角色摸两张牌，然后每名目标角色弃置两张牌。",

	["known_both"] = "知己知彼",
	[":known_both"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：一名其他角色。<br />作用效果：你观看目标角色的一张暗置的武将牌或其手牌。<br />◆你可以重铸此牌。",
	["#KnownBothView"] = "%from 观看了 %to 的 %arg" ,
	["$KnownBothViewGeneral"] = "%from 观看了 %to 的 %arg2，为 %arg" ,
	
	["befriend_attacking"] = "远交近攻",
	[":befriend_attacking"] = "法术牌<br />使用时机：出牌阶段。<br />使用目标：有明置武将牌且与你势力不同的一名角色。<br />作用效果：目标角色摸一张牌，然后你摸三张牌。",
